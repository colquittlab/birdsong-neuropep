...
```{r}
labels = list(bu88bu26 =c("c", "d", "e", "w"))
```
...
## Calc FF
```{r}
params1 = data.frame(bird="bu88bu26",
                    labels =labels[[1]],
                    lbuffer=c(.01, .015, .025, .02),
                  #  rbuffer = c(.02, .03, .0),
                    freq_lower = c(3.5, 3, 4, 2),
                    freq_upper = c( 4.5, 4, 5, 3),
                    stringsAsFactors = F)
...
ffreq1 = ffreq1 %>% left_join(syls %>% select(wav, onsets, labels, pred_ent, duration))
...
ffreq3_ds = ffreq3 %>% mutate(date = as.Date(date)) %>% 
  inner_join(ds_log) %>%
  mutate(period = if_else(parsed_time < ds_datetime_start, "PRE", "DS"),
...
ffreq2_ds_group = ffreq2_ds_group %>% left_join(info) %>%
  distinct(labels, period, .keep_all=T) %>% mutate(period = factor(period, levels=c("PRE", "DS", "POST")))
```
