...
## Calc FF
```{r}
params1 = data.frame(bird="or45pu98",
                    labels =labels[[1]],
                    lbuffer=c(.055, .03),
                  #  rbuffer = c(.02, .03, .0),
                    freq_lower = c(2.5, 2),
...
ffreq2_group = ffreq2 %>% 
  group_by(labels, labels_con, rel_date) %>%
...
gg = gg + facet_grid(labels_con~., scales="free_y")
gg = gg + theme_classic()
...
ffreq2_ds = ffreq3_ds %>% 
  group_by(labels, labels_con, period) %>%
  summarize_at(vars(ff, ff_loess, duration),
               list(mean= "mean_na", cv = "calc_cv"))
  #summarize(ff_mean = mean(ff, na.rm=T),
  ##          ff_cv = sd(ff, na.rm=T) / ff_mean,
...
ffreq2_ds_group = ffreq2_ds_group %>% left_join(info) %>%
  distinct(labels, labels_con, period, .keep_all=T) %>% mutate(period = factor(period, levels=c("PRE", "DS", "POST")))
```

```{r}
labels_con_to_use = c("cD", "dD", "aV")
ffreq3_ds %>% select(labels_con, period) %>% table()
