...
```{r}
gg = ggplot(syls1 %>% group_by(labels) %>% sample_frac(.2) %>% ungroup(), aes(amp_mean_log2, went_mean_log2)) + geom_point(alpha=.1) + facet_wrap(~labels)
gg
```

```{r}
vars_cur = vars(went_mean_log2, went_mean_log2_res, amp_mean_log2, duration, 
...
### Across time
```{r}
values = c("duration_z", "went_mean_log2_z", "went_mean_log2_res_z",  "went_mean_log2_res2_z", "mean_pitch_goodness_z", "mean_delta_spectral_flatness_z", "amp_mean_log2_z", "mean_pitch_z", "mean_delta_pitch_z")
...
```{r}
labels = c(unique(syls1$labels))
labels = labels[!(labels %in% c("n"))]
walk(values, function(value) {
  print(value)
  walk(labels, function(label) {
    #print(label)
    tmp = syls1a %>% filter(labels==label)
    if (nrow(tmp)==0)
      return(NULL)
    gg = plot_across_time_current(tmp , value)
    gg = gg + hl
    ...
    gg = gg + stat_smooth(se=F, fullrange=F, span=.5, method="loess")
    gg = gg + scale_color_manual(values=colors)
     gg = gg + labs(title=sprintf("%s - %s", value, label))
     
   #print(gg)
    ggsave(paste(by_day, sprintf("%s_%s_time.pdf", value, label), sep="/"), width=fig_width, height=fig_height)
})
})
```
...
## Calc FF
```{r}
params1 = data.frame(bird="pu30bk80",
                    labels = c("c", "f", "w"),
                    lbuffer=c(.015, .025, .02),
                    freq_lower = c(3, 3.5, 2),
                    freq_upper = c(4, 4.5, 3),
                    stringsAsFactors = F)
params = params1
...
info1 = info %>% filter(rel_date>=-4) #%>% head(200)
drop = F
redo = T
wl = 1024
...
#ffreq1 = ffreq1 %>% left_join(syls1 %>% select(mat, onsets, labels, pred_ent, labels_con, labels_div))
ffreq1 = ffreq1 %>% left_join(syls1 %>% select(mat, onsets, labels, pred_ent))
...
## Summaries
```{r}
perc_change = function(x, baseline) {
...
ffreq3 = ffreq3 %>%
  group_by(labels) %>% 
  mutate(ff_z= zscore_my(ff, ff[rel_mtime<=0]))
...
gg = ggplot(tmp, aes(period, ff_cv, fill=period)) 
...
save_plot(file.path(plot_dir, "ff_cv.pdf"), gg, base_width=6, base_height=nexpts, ncol=1, nrow=1)
...
gg = ggplot(tmp, aes(labels, ff_cv_change, fill=period)) 
...
gg
nexpts = length(unique(tmp$labels))
save_plot(file.path(plot_dir, "ff_cv_change.pdf"), gg, base_width=nexpts*3, base_height=2, ncol=1, nrow=1)
...
## Rolling CV
```{r}
ffreq_cv = ffreq3 %>% group_by(labels) %>% arrange(mtime_onsets) %>% do({
  d = .
  res = calc_rolling_func(d$ff, 5, direction = "backward"  )
  res11 = calc_rolling_func(d$ff, 11, direction = "backward" )
  data.frame(., ff_cv=res, ff_cv11 = res11)
})
ffreq_cv = ffreq_cv %>% group_by(labels) %>% 
  mutate(ff_cv_z = zscore_my(ff_cv, ff_cv[period=="PRE"]),
         ff_cv11_z = zscore_my(ff_cv11, ff_cv11[period=="PRE"]),
         ff_cv_perc = 100 * (ff_cv - mean(ff_cv[period=="PRE"])) / mean(ff_cv[period=="PRE"]))

ffreq3 = ffreq3 %>% group_by(labels) %>%
  do({
    pre = mean(.$ff[.$period=="PRE"])
    ff_perc = 100 * (.$ff - pre) / pre
    data.frame(., ff_perc=ff_perc)
  })

ffreq3 = ffreq3 %>% ungroup()
```
## Within bout CV
```{r}
...
```{r}
ffreq1 %>% filter(labels=="d", ff > 3.6, rel_date==-1) %>% select(wav, onsets, ff) %>% arrange(desc(ff)) %>% head(50)
```
...
# walk(ff_labels, function(label) {
#   tmp = ffreq3 %>% filter(labels==label) %>% 
#     group_by(labels_con, expt) %>% filter(n()>500)
#   gg = ggplot(tmp, aes_string("mtime", value))
#   gg = gg + geom_point(aes(color=log.drug), alpha=.1)
#   gg = gg + scale_color_manual(values=colors)
#   gg = gg + stat_smooth(se=F, fullrange=F, span=.5, method="loess")
#   gg = gg + labs(title=label)
#   gg = gg + facet_grid(labels_con~.)
#   gg = gg + guides(color = guide_legend(override.aes=list(alpha=1)))
#   print(gg)
...
walk(ff_labels, function(label) {
  ...
  save_plot(file.path(by_day, sprintf("%s_%s_time.pdf", value, label)), gg, base_width=1, base_height=3, ncol=ndays, nrow=1)
...
walk(values, function(value) {
  ...
  nrows = ceiling(n_expts / ncolumns)
    ...
    gg = gg + labs(title=sprintf("%s - %s", value, label), y = "FF CV")
    print(gg)
     save_plot(file.path(by_day, sprintf("%s_%s_bout_time.pdf", value, label)), gg, base_width=2, base_height=2, ncol=ncolumns, nrow=nrows)
  })
})
...
walk(values, function(value) {
  walk(ff_labels, function(label) {
    ...
    gg = gg + ylim(0, .06)
    print(gg)
    save_plot(paste(across_days, sprintf("%s_%s_bout_days.pdf", value, label),sep="/"), gg, base_width = 1.5, base_height = 4, ncol=total_days, limitsize=F)
  })
})

```

## Calculate baseline circadian effects
```{r}
baseline_to_use = c("PBS-control (4)", "PBS-control (5)",  "PBS-control (7)", "PBS-control (8)", 
                    "no-cannulae (1)",  "no-cannulae (2)",  "no-cannulae (3)",
                    "no-probe (2)", "no-probe (3)", "no-probe (4)")

ffreq_cv_base = ffreq_cv %>% filter(expt %in% baseline_to_use) %>% 
  group_by(expt) %>%
  mutate(ff_cv_mean = mean(ff_cv, na.rm=T),
         ff_cv_zero = ff_cv - ff_cv_mean)

ffreq_cv_base$time_period = hms(ffreq_cv_base$time)

ffreq_cv_base_reduce = ffreq_cv_base %>% group_by(time_h) %>% summarize()

spline_fn = with(ffreq_cv_base, splinefun(time_h, ff_cv_zero))

normalize_by_spline = function(y, x, sp_fn) {
  y_mean = mean(y, na.rm=T)
  print(head(y_mean))
  y_zero = y - y_mean
  print(head(y_zero))
  y_zero_sp = y_zero - sp_fn(x)
  print(head(y_zero_sp))
  y_zero_sp + y_mean
}
```
...
```{r}
hour_range = seq(7, 19)
spline_test = spline_fn(hour_range)
plot(spline_test)
```
```{r}


ffreq_cv = ffreq_cv %>% group_by(expt) %>% 
  mutate(ff_cv_sp = normalize_by_spline(ff_cv, time_h, spline_fn),
         ff_cv11_sp = normalize_by_spline(ff_cv11, time_h, spline_fn))
```

```{r}
values = c("ff_cv_sp", "ff_cv11_sp")
walk(values, function(value) {
  walk(ff_labels, function(label) {
  ...
  nrows = ceiling(n_expts / ncolumns)
    ...
    gg = gg + ylim(0, .06)
    gg = gg + stat_smooth(se=F, fullrange=F, span=.5, method="loess")
    gg = gg + labs(title=sprintf("%s - %s", value, label), y = "FF CV, circadian adjusted")
    print(gg)
     save_plot(file.path(by_day, sprintf("%s_%s_time.pdf", value, label)), gg, base_width=2, base_height=2, ncol=ncolumns, nrow=nrows)
  })
})
```
...
ffreq_cv_base = ffreq_cv %>% filter(expt %in% baseline_to_use) %>% 
  ...
  mutate(ff_cv_mean = mean(ff_cv, na.rm=T),
         ff_cv_zero = ff_cv - ff_cv_mean) %>%
  group_by(time_h, expt) %>%
  summarize(ff_cv_zero_mean = mean(ff_cv_zero, na.rm=T)) %>%
  ungroup()

ffreq_cv = ffreq_cv %>% left_join(ffreq_cv_base %>% select(time_h, ff_cv_zero_mean)) %>%
  distinct(id, .keep_all=T) %>%
  mutate(ff_cv_cir_adjust = ff_cv - ff_cv_zero_mean,
         ff_cv11_cir_adjust = ff_cv11 - ff_cv_zero_mean)
# normalize_by_spline = function(y, x, sp_fn) {
#   y_mean = mean(y, na.rm=T)
#   print(head(y_mean))
#   y_zero = y - y_mean
...
```{r}
gg = ggplot(ffreq_cv_base, aes(time_h, ff_cv_zero_mean))
gg = gg + geom_point()
gg = gg + facet_wrap(~expt)
gg
```
...
```{r}
values = c("ff_cv_cir_adjust", "ff_cv11_cir_adjust")
walk(values, function(value) {
  walk(ff_labels, function(label) {
    tmp = ffreq_cv %>% filter(labels==label, expt %in% filts)
    n_expts = length(unique(as.character(tmp$expt)))
...
map(songs1, function(s) {
  ...
  #expt = s$expt
  rel_date = s$rel_date
  fname = file.path(song_dir, sprintf("%s_rel_date%s.pdf", wav_prefix, rel_date))
  if (nrow(s$motifs) > 0) {
