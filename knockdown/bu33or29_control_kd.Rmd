...
# Set up data db
```{r}
db = dbConnect(RSQLite::SQLite(), paste(data_dir, sprintf("%s.db", bird), sep="/") )
...
```{r}
ggplot(syls1, aes(log(pred_ent))) + 
  geom_density() + 
  facet_wrap(~labels)
```
```{r}
syls1 = syls1 %>% group_by(labels) %>% mutate(pred_ent_log = log(pred_ent)) 
syls1_pred_ent_stat = syls1 %>%
  group_by(labels) %>% 
  summarize(pred_ent_log_mean = mean(pred_ent_log),
            pred_ent_log_sd = sd(pred_ent_log),
            pred_ent_log_thresh = pred_ent_log_mean + pred_ent_log_sd*2,
            pred_ent_mean = mean(pred_ent),
            pred_ent_sd = sd(pred_ent),
            pred_ent_thresh = pred_ent_mean + pred_ent_sd*2)
syls1 = syls1 %>% left_join(syls1_pred_ent_stat %>% select(labels, pred_ent_log_thresh, pred_ent_thresh))
```

```{r}
syls1 %>% group_by(labels) %>% summarize(thresh_log = sum(pred_ent_log>pred_ent_log_thresh),
                                         thresh = sum(pred_ent > pred_ent_thresh),
                                         total = n())
```

```{r}
ggplot(syls1 %>% filter(pred_ent_log < pred_ent_log_thresh), aes(pred_ent)) + 
  geom_density() + 
  facet_wrap(~labels)

ggplot(syls1 %>% filter(pred_ent < pred_ent_thresh), aes(pred_ent)) + 
  geom_density() + 
  facet_wrap(~labels)
```
...
```{r}
vec = syls1 %>% filter(labels=="w") %>% ungroup() %>% select(pred_ent) %>% unlist() %>% c()
fits = map(c("gamma", "norm", "unif", "exp"), function(x) fitdist(vec, x))
denscomp(fits)
gofstat(fits[[1]])
```

```{r}
fit = fitdist(vec, "gamma")
pfit = pgamma(vec, shape=fit$estimate[1], rate=fit$estimate[2])
```

```{r}
syls1 = syls1 %>% group_by(labels) %>% do({
  d = .
  fit = fitdist(d$pred_ent, "gamma")
  pfit = pgamma(d$pred_ent, shape=fit$estimate[1], rate=fit$estimate[2])
  data.frame(d, pfit=pfit)
})
```

```{r}
gg = ggplot(syls1 %>% filter(labels=="w"), aes(pred_ent, pfit)) + 
  geom_point()
gg
```
```{r}
syls1 %>% group_by(labels) %>% summarize(n_thresh = sum(pfit>.9)/ n(),
                                          total = n())
```
```{r}
ggplot(syls1 %>% filter(pfit < .9), aes(pred_ent)) + 
  geom_density() + 
  facet_wrap(~labels)
```
...
## Calc FF
```{r}
params1 = data.frame(bird="bu33or29",
                    labels =labels[[1]],
                    lbuffer=c(.02, .02),
                  #  rbuffer = c(.02, .03, .0),
                    freq_lower = c(3, 3.5),
...
if (!dbExistsTable(db, tname) || redo) {
  
  if (drop && dbExistsTable(db, tname))
    dbRemoveTable(db, tname)
