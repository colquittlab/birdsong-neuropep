...
## Calc FF
```{r}
params1 = data.frame(bird="rd25gr70",
                    labels =labels[[1]],
                    lbuffer=c(.01, .015, .05),
                    rbuffer = c(.02, .03, .07),
                    freq_lower = c(3, 3, 1),
...
```
strong positive relationship between duration and ff for syllable b, weaker for syllable c, none for g
```{r fig.height=10}
...
ffreq2 = ffreq3 %>% 
  ...
  summarize_at(vars(ff, ff_loess, duration),
...
ffreq2_base = ffreq2 %>% filter(rel_date <=0) %>% 
  group_by(labels) %>% 
  summarize_at(vars(ff_mean, ff_cv, ff_loess_mean, ff_loess_cv, duration_mean, duration_cv), list("mean" = mean))
...
save_plot(file.path(freq_dir, "ff_cv_ff2.pdf"), gg, base_width=6, base_height=nexpts, ncol=1, nrow=1)
...
gg
nexpts = length(unique(tmp$labels))
save_plot(file.path(freq_dir, "ff_loess_cv_ff2.pdf"), gg, base_width=6, base_height=nexpts, ncol=1, nrow=1)
...
save_plot(file.path(freq_dir, "ff_cv_change_ff2.pdf"), gg, base_width=nexpts/2, base_height=2, ncol=1, nrow=nlabels)
...
nexpts = length(unique(tmp$rel_date))
nlabels = length(unique(tmp$labels))
save_plot(file.path(freq_dir, "ff_loess_cv_change_ff2.pdf"), gg, base_width=nexpts/2, base_height=2, ncol=1, nrow=nlabels)
...
tmp1 = tmp %>% filter(rel_date %in% c(-2, -1, 3, 4,5, 6)) %>%
