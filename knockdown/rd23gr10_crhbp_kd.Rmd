...
# labels = labels[!(labels %in% c("n"))]
# 
# values_no_z = c("duration", "went_mean_log2", "went_mean_log2_res", "mean_pitch_goodness", "mean_delta_spectral_flatness", "amp_mean_log2", "mean_pitch", "mean_delta_pitch")
# walk(values_no_z, function(value) {
#   print(value)
#   walk(labels, function(label) {
#     print(label)
#     tmp = syls1a %>% filter(labels==label) %>%
...
#     ggsave(paste(by_day, sprintf("%s_%s_time.pdf", value, label), sep="/"), width=fig_width, height=fig_height)
#     
#     gg = ggplot(tmp %>% group_by(expt), aes_string("mtime", value))
...
#   
#     #ggsave(paste(across_days, sprintf("%s_%s_days.pdf", value, label),sep="/"), width=total_days / 2, height=4 )
#   
# })
# })
```
...
## Calc FF
```{r}
params1 = data.frame(bird="rd23gr10",
                    labels = c("d", "w", "g"),
                    lbuffer=c(.02, .01, .05),
                    freq_lower = c(3, 3, 1.5),
                    freq_upper = c(4, 4, 2.5),
                    stringsAsFactors = F)
params = params1
...
save_plot(file.path(plot_dir, "ff_cv_change.pdf"), gg, base_width=nexpts/2, base_height=2, ncol=1, nrow=nlabels)
...
walk(ff_labels, function(label) {
  ...
  save_plot(file.path(by_day, sprintf("%s_%s_time.pdf", value, label)), gg, base_width=.75, base_height=3, ncol=ndays, nrow=1)
