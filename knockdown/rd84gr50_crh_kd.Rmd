...
## Calc FF
```{r}
params1 = data.frame(bird="rd84gr50",
                    labels =labels[[1]],
                    lbuffer=c(.02, .06),
                    freq_lower = c(3, 1.5),
                    freq_upper = c( 4, 2.5),
...
ffreq1 = ffreq1 %>% left_join(syls1 %>% select(mat, onsets, labels, pred_ent, duration))
...
ffreq1_n = ffreq1 %>% group_by(rel_date, labels) %>% summarize(n=n())
...
## FF density
```{r}
tmp = ffreq3 %>% filter(rel_date>=-4, rel_date<=8)
...
save_plot(file.path(freq_dir, "ff_z_density_by_day.pdf"), gg, base_height=3, base_aspect_ratio = 1.3, ncol=ncols, nrow=1)
...
ncols = length(unique(ffreq3$labels))
save_plot(file.path(freq_dir, "ff_density_by_day.pdf"), gg, base_height=3, base_aspect_ratio = 1.3, ncol=ncols, nrow=1)
```
