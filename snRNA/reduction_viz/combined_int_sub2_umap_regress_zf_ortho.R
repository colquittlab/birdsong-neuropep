...
dir_root = "~/nest/rstudio/neuropeptide/snRNA/"
positions = c("ra", "hvc")
sn_input_dir = "~/nest_common/sc_datasets/colquitt_2021/"
data_fname = file.path(sn_input_dir, "HVC_RA_paper.qs")

dir_out_root = "~/nest/rstudio/neuropeptide/snRNA"
red_dir = file.path(dir_out_root, "reduction_viz")
...
md = FetchData(obj_int_filt, "cluster_int_sub2") %>% 
  ...
  left_join(ct_df)  %>%
  column_to_rownames() %>%
  select(celltype, celltype_class1, text_colors)
...
nclusters = length(unique(obj_int_filt$celltype_class1))
cols = DiscretePalette_scCustomize(nclusters, palette = "ditto_seq", shuffle_pal = T, seed = 123)
gg = DimPlot_scCustom(obj_int_filt, reduction=reduction.name, group.by=ca, label=F, repel = T, colors_use = cols, raster = F, raster.dpi = c(128, 128)) +
  theme(axis.line = element_blank(),
        axis.ticks = element_blank(),
        axis.text = element_blank(),
...
reduction.name_column = sub("\\.", "", reduction.name)

gg = DimPlot_scCustom(obj_int_filt, reduction=reduction.name, group.by=ca, label=F, repel = T, colors_use = position_pretty_colors[c("HVC", "RA")] ) +
  theme(axis.line = element_blank(),
        axis.ticks = element_blank(),
        axis.text = element_blank(),
        axis.title = element_blank(),
...
# Colored by region, split by region -------------------------------------------------------------------------
...
gg = DimPlot_scCustom(obj_int_filt, reduction=reduction.name, group.by=ca, label=F, repel = T, split.by = ca, colors_use = position_pretty_colors[c("HVC", "RA")], split_seurat=T ) +
  theme(axis.line = element_blank(),
        axis.ticks = element_blank(),
        axis.text = element_blank(),
        axis.title = element_blank(),
...
gg
save_plot(file.path(figures_dir, sprintf("umap_%s_%s_split.pdf", reduction.name, ca)), gg, base_height=6, base_asp =1, ncol=2 )
save_plot(file.path(figures_dir, sprintf("umap_%s_%s_split.png", reduction.name, ca)), gg, base_height=6, base_asp =1, ncol=2 )
